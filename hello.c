...
#include <linux/uaccess.h> //for 4.14 kernel
...
{
	int result = 0;
	int size, value, *port1, *port2;
	char *ch;
	void __iomem *p;
	resource_size_t resource = 0;
	...
	switch(cmd) {
		...
		case 3:
			...
			break;
		case 4:
			resource = (resource_size_t)*port1;
			resource &= 0xffffffff;
			//printk("resource=0x%llx\n", resource);
			//result = request_mem_region(resource, 4, "devport-mem");
			result = 0;
			if (result) {
				printk("request mem region fails\n");
				goto done;
			} else {
				p = ioremap_nocache(resource, 4);
				//printk("ioremaaap p=0x%p\n", p);
			}
			value = *(int*)p;
			//printk("value=0x%x\n", value);
			iounmap(resource);
			//release_mem_region(resource, 4);
		case 6:
			resource = (resource_size_t)*port1;
			resource &= 0xffffffff;
			result = 0;
